...
namespace Re4QuadExtremeEditor.src.Controls
{
    public partial class ObjectMoveControl : UserControl
    {
        ...
        MoveObjCombos LastCombos = MoveObjCombos.Null;
        ...
        public void UpdateSelection()
        {
            ...
            if (SelectedNodes.Count > 0)
            {
                for (int i = 0; i < SelectedNodes.Count; i++)
                {
                    if (SelectedNodes[i] is Object3D obj)
                    {
                        ...
                        else if (parent is NewAge_FSE_NodeGroup || parent is NewAge_ESAR_NodeGroup)
                        {
                            combos |= MoveObjCombos.NewAgeTriggerZone;
                        }
                        else if (parent is NewAge_EMI_NodeGroup || parent is NewAge_ESE_NodeGroup || parent is NewAge_LIT_Entrys_NodeGroup || parent is NewAge_EFF_EffectEntry_NodeGroup)
                        {
                            combos |= MoveObjCombos.NewAgePositionOnly;
                        }
                    }
                }
            ...
            else if (combos == MoveObjCombos.NewAgeTriggerZone)
            {
                comboBoxMoveMode.Items.Add(new MoveObjTypeObjForListBox(MoveObjType.SquareMoveTriggerZoneAllPointsXZ_VerticalMoveTriggerZoneY_Horizontal1ChangeTriggerZoneHeight_Horizontal2RotationZoneY_Horizontal3ScaleAll, Lang.GetText(eLang.MoveMode_TriggerZone_MoveAll)));
                comboBoxMoveMode.Items.Add(new MoveObjTypeObjForListBox(MoveObjType.SquareMoveTriggerZonePoint0XZ_VerticalMoveTriggerZoneY_Horizontal1ChangeTriggerZoneHeight_Horizontal23None, Lang.GetText(eLang.MoveMode_TriggerZone_Point0)));
                comboBoxMoveMode.Items.Add(new MoveObjTypeObjForListBox(MoveObjType.SquareMoveTriggerZonePoint1XZ_VerticalMoveTriggerZoneY_Horizontal1ChangeTriggerZoneHeight_Horizontal23None, Lang.GetText(eLang.MoveMode_TriggerZone_Point1)));
            ...
            else if (combos == MoveObjCombos.NewAgePositionOnly)
            {
            ...
            // If the selection moved to a different object category (e.g. from an AEV/trigger zone
            // to an Item), don't keep whatever move-mode was selected for the old category — even if
            // that exact mode also happens to appear in the new category's list (like TriggerZone
            // modes appearing for both SpecialTriggerZone and Item). Reset to that category's default.
            if (combos != LastCombos)
            {
                ...
                foreach (TreeNode item in DataBase.SelectedNodes.Values)
                {
                    if (item is Object3D obj && item.Parent is TreeNodeGroup)
                    {
                        ...
                        else if (MoveObjTypeSelected.HasFlag(MoveObjType._SquareMoveTriggerZone))
                        {
                            ...
                            else if (obj.Parent is NewAge_FSE_NodeGroup fseGroup)
                            {
                                category = (SpecialZoneCategory)fseGroup.MethodsForGL.GetZoneCategory(obj.ObjLineRef);
                            }
                            else if (obj.Parent is NewAge_ESAR_NodeGroup esarGroup)
                            {
                                category = (SpecialZoneCategory)esarGroup.MethodsForGL.GetZoneCategory(obj.ObjLineRef);
                            }
